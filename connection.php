...
function check_name($from, $to, $amount)
{
    ...
    $user = 'root';
    $password = '';
    $dns = 'mysql:host=localhost;dbname=basic_bank';
    ...
    try {
        ...
        $sql = 'SELECT amount FROM user_data WHERE User_Name=:from';
        $stmt = $db->prepare($sql);
        ...
        $st1 = $db->prepare(
            ...
            `Amount`= Amount - :amt where User_Name = :fro "
        );
        $st2 = $db->prepare(
            "update user_data set
            `Amount`= Amount + :amt where User_Name = :to "
        );

        $st3 = $db->prepare(
            "INSERT INTO `transaction` ( `FromUser`, `ToUser`, `Amount`) VALUES ( :from, :to, :amount)"
        );
        ...
        if (!$st3->rowCount())
            throw new Exception('Could not insert data into Transaction Entry!');
